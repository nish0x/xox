...
// Compressed formats
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFBC1_RGB] = {
  format: COMPRESSED_RGB_S3TC_DXT1_EXT,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFBC3_RGBA] = {
  format: COMPRESSED_RGBA_S3TC_DXT5_EXT,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFBC7_RGBA] = {
  format: COMPRESSED_RGBA_BPTC_UNORM_EXT,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFETC1_RGB] = {
  format: COMPRESSED_RGB_ETC1_WEBGL,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFETC2_RGBA] = {
  format: COMPRESSED_RGBA8_ETC2_EAC,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFASTC_4x4_RGBA] = {
  format: COMPRESSED_RGBA_ASTC_4x4_KHR,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFPVRTC1_4_RGB] = {
  format: COMPRESSED_RGB_PVRTC_4BPPV1_IMG,
};
BASIS_WEBGL_FORMAT_MAP[BASIS_FORMAT.cTFPVRTC1_4_RGBA] = {
...
function resolveTextureFormat(supportedFormats, hasAlpha) {
  if (hasAlpha) {
    if (supportedFormats.etc2) {
      return BASIS_FORMAT.cTFETC2_RGBA;
    } else if (supportedFormats.bptc) {
      return BASIS_FORMAT.cTFBC7_RGBA;
    } else if (supportedFormats.s3tc) {
      return BASIS_FORMAT.cTFBC3_RGBA;
    } else if (supportedFormats.astc) {
      return BASIS_FORMAT.cTFASTC_4x4_RGBA;
    } else if (supportedFormats.pvrtc) {
      return BASIS_FORMAT.cTFPVRTC1_4_RGBA;
    } else if (supportedFormats.etc1) {
      return BASIS_FORMAT.cTFETC1_RGB;
    } else {
    ...
    if (supportedFormats.etc1) {
      ...
      return BASIS_FORMAT.cTFBC1_RGB;
    } else if (supportedFormats.etc2) {
      return BASIS_FORMAT.cTFETC2_RGBA;
    } else if (supportedFormats.astc) {
      return BASIS_FORMAT.cTFASTC_4x4_RGBA;
    } else if (supportedFormats.pvrtc) {
      return BASIS_FORMAT.cTFPVRTC1_4_RGB;
    } else {
      return BASIS_FORMAT.cTFRGB565;
    }
  }
}
...
function transcode(
  ...
  for (let mipLevel = 0; mipLevel < levels; ++mipLevel) {
    ...
    let transcodeSize = ktx2File.getImageTranscodedSizeInBytes(
      mipLevel,
      IMAGE_INDEX,
      0,
      basisFormat,
